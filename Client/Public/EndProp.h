...
BEGIN(Client)
class CEndProp final : public CProp
{
public:
	enum COLLIDERDOOR {COLLIDER_AABB0, COLLIDER_AABB1, COLLIDER_END};

private: 
	explicit		 CEndProp(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit		 CEndProp(const CEndProp& Prototype);
	virtual			~CEndProp() = default;
...
public:
	static CEndProp*			Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
