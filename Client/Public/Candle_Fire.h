...
BEGIN(Client)
class CCandle_Fire final : public CProp
{
public:
	enum COLLIDERDOOR {COLLIDER_AABB0, COLLIDER_AABB1, COLLIDER_END};

private: 
	explicit		 CCandle_Fire(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit		 CCandle_Fire(const CCandle_Fire& Prototype);
	virtual			~CCandle_Fire() = default;
...
public:
	static CCandle_Fire*			Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
...
private:
	...
	_bool*					m_pCandleFireOn_Off = { nullptr };
