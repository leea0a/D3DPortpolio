...
void CAnimation::Store_CurrentAnim_KeyFrame_State(const vector<class CBone*>& Bones)
{
	//오염방지 
	m_mapStartKeyFrames.clear();

	KeyFramePosOfStartAnim = m_fCurrentTrackPosition;
...
void CAnimation::Store_AnimToChange_KeyFrame_State(const vector<class CBone*>& Bones, CAnimation* NextAnim)
{
	...
	KeyFramePosOfNextAnim = fDistance+KeyFramePosOfStartAnim;
...
{
	/*vector<_bool> IsFinished;
	IsFinished.resize(m_mapLastKeyFrames.size());
	int iIndex = 0;*/
	...
	{
		KeyFramePosOfStartAnim = 0;
		KeyFramePosOfNextAnim = 0;
		m_mapStartKeyFrames.clear();
		m_mapLastKeyFrames.clear();
		m_fCurrentTrackPosition = 0.f;
	}
...
_bool CAnimation::Update_TransformMatrix_ForMotionChange(const vector<class CBone*>& Bones, _uint iBoneIndex,KEYFRAME Start, KEYFRAME Last)
{
	...
	//	*pKeyFrameIndex = 0;
		...
		if ((m_fCurrentTrackPosition - KeyFramePosOfStartAnim) > fDistance)
			return true; //끝나면 true 
		
		_float fRatio = (m_fCurrentTrackPosition - KeyFramePosOfStartAnim) / fDistance;
